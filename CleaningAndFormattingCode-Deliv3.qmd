...
1.  Read in [Kitchen Nightmares data link](https://github.com/DataPreProcessing-FALL2024/Deli3Test/raw/refs/heads/main/Data/messy_kitchen_nightmares.csv) from Github repository.

```{r}
library(dplyr) 
library(tidyverse)  
kn_link="https://github.com/DataPreProcessing-FALL2024/Deli3Test/raw/refs/heads/main/Data/messy_kitchen_nightmares.csv"
```
...
27. Saving the cleaned Kitchen Nightmares data set to 'DataCleanAndFormatted' folder.
...
if (!dir.exists(folder)) {
  # Create the folder
  dir.create(folder)
  write.csv(kn_data,file.path(folder,"Clean_KitchenNightmares_Data.csv"),
            row.names=F)
...
1.  Read in [Clean_KitchenNightmares_Data](https://github.com/DataPreProcessing-FALL2024/Deli3Test/raw/refs/heads/main/DataCleanAndFormatted/Clean_KitchenNightmares_Data.csv) from Github link.
...
7.  Finally, we'll save the newly formatted Kitchen Nightmares data to the DataCleanAndFormatted folder as an RDS file.
...
if (!dir.exists(folder)) {
  # Create the folder
  dir.create(folder)
  saveRDS(clean_kn,file.path(folder,"Formatted_KitchenNightmares_Data.RDS"))
...
library(dplyr)
library(tidyverse)
hh_link = "https://github.com/DataPreProcessing-FALL2024/Deli3Test/raw/refs/heads/main/Data/messy_hotel_hell.csv"
```
...
24. Saving the cleaned Hotel Hell data set to 'DataCleanAndFormatted' folder.
...
if (!dir.exists(folder)) {
  # Create the folder
  dir.create(folder)
  write.csv(hh_data,file.path(folder,"Clean_HotelHell_Data.csv"),
            row.names=F)
...
7.  Finally, we'll save the newly formatted Hotel Hell data to the DataCleanAndFormatted folder as an RDS file.
...
if (!dir.exists(folder)) {
  # Create the folder
  dir.create(folder)
  saveRDS(clean_hh,file.path(folder,"Formatted_HotelHell_Data.RDS"))
